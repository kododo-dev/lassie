...
### Discovered during Phase 3 production verification (three fixes, all confirmed only observable in production)
...
- **`NavigationManager.NavigateTo()` with a leading `/` ignores `<base href>`.** `Login.razor`, `Logout.razor`, and `RedirectToLogin.razor` all originally called `NavigateTo("/", ...)` / `NavigateTo("/login", ...)` — a leading `/` resolves against the domain root per standard URI-combining rules, not against the configured base path, so in production this sent the browser to `kododo.dev/` (the shared VPS's other site) instead of `kododo.dev/lassie/`. Invisible in every local test since there's no prefix locally for the two paths to diverge. Fixed by using relative targets without a leading slash (`""` for home, `"login"` for the login page), which resolve against `NavigationManager.BaseUri` instead.
