...
Two practical consequences of U-shaped attention (Lost-in-the-Middle):
...
- Auto-memory is local to the machine and not a substitute for `AGENTS.md` — team-binding rules live in the repo; auto-memory is a personal, periodically-reviewable cache.

### Inner-loop hooks (deterministic feedback without prompting)

Mechanical, non-pickable checks belong in hooks (e.g. Claude Code's `PostToolUse`), not in the rule file. Settings template (`settings.json.template`) ships in the lesson pack as the wiring entry point. Keep procedural workflows (deeper review, release checklist, deploy on sandbox) in skills, and reserve hooks for deterministic tool signals.
